...
describe('useRecipeSearchForm', () => {
    ...
    describe('search', () => {
        ...
        test('カテゴリIDが一致するレシピを表示する', () => {
            ...
            const updatedList = updater(mockRecipes);
            expect(updatedList).toEqual([
                ...mockRecipes.map(r => (r.category.id === 1 || r.category.id === 2) ? { ...r, visible: true } : { ...r, visible: false }),
            ]);
        })
        ...
        test('キーワードとカテゴリIDの両方に一致するレシピを表示する', () => {
            ...
            const updatedList = updater(mockRecipes);
            expect(updatedList).toEqual([
                ...mockRecipes.map(r => (r.category.id === 1 && r.name.includes('レシピ4')) ? { ...r, visible: true } : { ...r, visible: false }),
            ]);
        })
    })
})
