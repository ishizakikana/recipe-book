...
>(
     ...
     * @returns 全レコード
     */
    const findAll = async (args?: PrismaQueryArgs): Promise<T[]> => {
     ...
     */
    const findAllByConditions = async (
     ...
     * @returns 作成されたレコード
     */
    const create = async (data: CreateData): Promise<T> => {
     ...
     * @returns 削除されたレコード
     */
    const deleteOnce = async (id: T['id']): Promise<T> => {
     ...
     */
    const deleteAll = async (ids: T['id'][]): Promise<{ count: number }> => {
